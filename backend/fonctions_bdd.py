...
from tables import Plans, Oeuvres, Entities, Points, Relations, Chunk, Criterias, Pregeneration, Oeuvre_Criterias, Generated_Guide, Criterias_Guide, Criterias_Pregeneration, QR_Code, Stats
...
def create_Pregeneration(pregeneration: Pregeneration, session: Session) -> Pregeneration:
...
def delete_Pregeneration(pregeneration_id: int, session: Session) -> bool:
    pregeneration = session.get(Pregeneration, pregeneration_id)
...
def update_Pregeneration(pregeneration_id: int, data: Pregeneration, session: Session) -> Optional[Pregeneration]:
    obj = session.get(Pregeneration, pregeneration_id)
    if not obj:
        return None
    for key, value in data.dict(exclude_unset=True).items():
        setattr(obj, key, value)
...
def get_Pregeneration(pregeneration_id: int, session: Session) -> Optional[Pregeneration]:
    return session.get(Pregeneration, pregeneration_id)


def get_all_Pregeneration(session: Session) -> List[Pregeneration]:
    return session.exec(select(Pregeneration)).all()
